...
UCfile=UCdir+"usgsCorr_"+dataname+".mat"
Datafile=UCdir+"dataset_"+dataname+".mat"
...
for j in range(0,nc):
    for i in range(0,n):
        ...
        for k in range(0,10):
            ...
            predSel=predList[i]
            Yp,Yptrain,regTreeModel,fitModelList,predind=\
                SSRS.RegressionTree(X_train[:,predSel],X_test[:,predSel],Y_train[:,[j]],Y_test[:,[j]],regTreeModel,fitModel,predName,
...
def testModel(predListTest,cc):
    ...
    for k in range(0, nit):
        ...
        for j in range(0, nmodel):
            for i in range(0, testsize.__len__()):
                ind = range(0, nind)
                X_train, X_test, Y_train, Y_test, ind_train, ind_test = \
                    cross_validation.train_test_split(Xn, dist[:, [cc]], ind, test_size=testsize[i], random_state=k)
                regTreeModel = tree.DecisionTreeRegressor(max_leaf_nodes=20, min_samples_leaf=20)
                fitModel = linear_model.LinearRegression()
    ...
    for k in range(0, nit):
        ...
        for i in range(0, 18):
            ind = range(0, nind)
            X_train, X_test, Y_train, Y_test, ind_train, ind_test = \
                cross_validation.train_test_split(Xn, dist[:, [cc]], ind, test_size=0.2, random_state=k)
    ...
    for i in range(0, 18):
        ...
        X_test = Xn[ind_test, :]
        Y_train = np.reshape(dist[ind_train, cc], (-1, 1))
        Y_test = np.reshape(dist[ind_test, cc], (-1, 1))
        for j in range(0, nmodel):
            regTreeModel = tree.DecisionTreeRegressor(max_leaf_nodes=20, min_samples_leaf=20)
            fitModel = linear_model.LinearRegression()
            predSel = predListTest[j]
...
for c in range(0,6):
    ss=np.sort(score[:,c])
    ssInd=np.argsort(score[:,c])
    predListTest=predList[ssInd[0:100]]

    testErr,trainErr1_huc2,testErr1_huc2,trainErr1_huc2_rt,testErr1_huc2_rt,\
    trainErr2_huc2,testErr2_huc2,hucTab=testModel(predListTest,c)
    predErrmatfile=UCdir+"py_predCV_"+dataname+"_c"+str(c)+".mat"
...
# PLOT TREE (Solo)
################################################################
treefolder=r"D:\Kuai\SSRS\tree\solo\\"
CVrankFile=treefolder+"CrossVal_rank.mat"
mat = sio.loadmat(CVrankFile)
rankInd=mat['rankInd']
predList=mat['predList'].flatten()

ntopmodel=5
ntoptree=5
nit=50
for cc in range(0,6):
    for km in range(0,ntopmodel):
        temp = np.ones([nit])
        predSel=predList[cc][km,:]
        for rs in range(0,nit):
            ind = range(0,nind)
            X_train, X_test, Y_train, Y_test, ind_train, ind_test = \
                cross_validation.train_test_split(Xn, dist, ind, test_size=0.2, random_state=rs)
            predName = [Field[i] for i in predSel]
            regTreeModel = tree.DecisionTreeRegressor(max_leaf_nodes=20,min_samples_leaf=20)
            fitModel = linear_model.LinearRegression()
            Yp, Yptrain, regTreeModel, fitModelList, predind = SSRS.RegressionTree(
                X_train[:, predSel], X_test[:, predSel],
                Y_train[:,[cc]], Y_test[:,[cc]], regTreeModel, fitModel,
                predName,doFitSelection=0, doMultiBand=1)
            rmse, rmse_band = SSRS.RMSECal(Yp, Y_test)
            temp[rs] = rmse
        tempSort = np.sort(temp)
        tempSortInd = np.argsort(temp)

        ## start print top trees
        for kt in range(0, ntoptree):
            treename = "c%s_model%s_tree%s" % (cc,km,kt)
            treeMatFile=treefolder+treename
            kk = tempSortInd[kt]
            ind = range(0,nind)
            X_train, X_test, Y_train, Y_test, ind_train, ind_test = \
            ...
            SSRS.Tree2Mat(regTree, predSel, Field, treeMatFile,
                          X, Xn, ind_train, ind_test,
                          Y_train[:, [cc]], Y_test[:,[cc]], Yp, Yptrain)
